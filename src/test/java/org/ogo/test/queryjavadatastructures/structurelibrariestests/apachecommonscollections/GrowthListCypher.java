...
public class GrowthListCypher {
    ...
    private Object sizeGrowthList(boolean isCypher) throws RemoteException {
        if (isCypher) {
            Object[] q = query("MATCH (:`org.apache.commons.collections4.list.GrowthList`)-->()-->()-[n]->() "
                    + "WHERE type(n) <> 'instanceof' " + "RETURN " + "CASE "
                    + "    WHEN COUNT(n) <> 0 THEN toInteger(type(max(n)))+1 " + "    ELSE 0 " + "END as size");
            return q[0];
        } /*
