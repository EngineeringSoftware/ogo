...
public abstract class OgoLinkedList {
    ...
    public static boolean contains(LinkedList list, Integer item, boolean defaultMode) throws RemoteException {
        if (defaultMode) {
            return list.contains(item);
        } else {
            return queryBool("MATCH ({$1})-[:first]->(m)-[*]->(n {value:" + item.intValue() + "}) RETURN True", list);
        }
    }
    ...
    public static int indexOf(LinkedList list, Integer item, boolean defaultMode) throws RemoteException {
        if (defaultMode) {
            ...
            return (int) queryLong("MATCH ({$1})-[:first]->(m)-[*]->(n {value:" + item.intValue()
