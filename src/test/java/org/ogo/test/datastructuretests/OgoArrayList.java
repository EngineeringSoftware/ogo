...
public abstract class OgoArrayList {
    ...
    public static boolean contains(ArrayList list, Integer item, boolean defaultMode, DataStructureTest test)
            throws RemoteException {
        if (defaultMode) {
            return list.contains(item);
        } else {
            return queryBool(test,
                    "MATCH ({$1})-[:elementData]->(m)-[*]->(n {value:" + item.intValue() + "}) RETURN True", list);
        }
    }
