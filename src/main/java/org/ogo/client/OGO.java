...
public abstract class OGO {
    ...
    /**
     * Instances of Classes belonging to these packages will be definitely included in the object graph
     */
    ...
    /**
     * Instances of Classes belonging to these packages will be definitely excluded from the object graph
     */
     ...
     *
     * <p>
     * This method must be called before executing any queries. It reads configuration from OGOProperties to determine
     * whether to use the in-memory graph query engine (OGO_Mem) or connect to a Neo4j database (OGO_Neo) via RMI for
     * query execution.
     *
     * @throws IOException
     *             if RMI registry connection fails
     * @throws NotBoundException
     *             if the Neo4JGraphQueryEngine is not bound in the RMI registry
     */
     ...
     *
     * @param className
     *            user given class name in CREATE query
     ...
     *
     * <p>
     * In OGO_Neo mode, the object graph is serialized to CSV files at this path for batch import into Neo4j. This path
     * is also used for profile data output when profiling is enabled.
     *
     * @param CsvPath
     *            the directory path where CSV files will be written
     *
     * @throws RemoteException
     *             if the remote graph interface is unavailable
     */
     ...
     *
     * @param cQuery
     *            the Cypher query string to execute on the entire object heap
     *
     * @return true if the query returns at least one result and the first result is a Boolean true, false otherwise
     *
     * @throws RemoteException
    ...
    /**
     * Executes an unbounded Cypher query with positional arguments and returns the first result as a boolean.
     *
     * @param cQuery
     *            the Cypher query string to execute on the entire object heap
     * @param objects
     *            variadic arguments used to expand positional arguments ($N, @N, []N) in the query string
     *
     * @return true if the query returns at least one result and the first result is a Boolean true, false otherwise
     *
     * @throws RemoteException
     ...
     *
     * @param root
     *            the root object that constrains query execution to the reachable subgraph
     * @param cQuery
     *            the Cypher query string to execute on the object graph
     *
     * @return true if the query returns at least one result and the first result is a Boolean true, false otherwise
     *
     * @throws RemoteException
    ...
    /**
     * Executes a bounded Cypher query with positional arguments and returns the first result as a boolean.
     *
     * @param root
     *            the root object that constrains query execution to the reachable subgraph
     * @param cQuery
     ...
     *             if the query execution fails
     */
    public static boolean queryBool(Object root, String cQuery, Object... objects) throws RemoteException {
    ...
    /**
     * Executes an unbounded Cypher query with positional arguments and returns the first result as an integer.
     *
     * @param cQuery
     *            the Cypher query string to execute on the entire object heap
     * @param objects
    ...
    /**
     * Executes a bounded Cypher query with positional arguments and returns the first result as an integer.
     *
     * @param root
     *            the root object that constrains query execution to the reachable subgraph
     * @param cQuery
    ...
    /**
     * Executes an unbounded Cypher query with positional arguments and returns the first result as a long.
     *
     * @param cQuery
     *            the Cypher query string to execute on the entire object heap
     * @param objects
    ...
    /**
     * Executes a bounded Cypher query with positional arguments and returns the first result as a long.
     *
     * @param root
     *            the root object that constrains query execution to the reachable subgraph
     * @param cQuery
     ...
     *             if the query execution fails
     */
    public static long queryLong(Object root, String cQuery, Object... objects) throws RemoteException {
        Object[] result = query(root, cQuery, objects);
        if (result.length > 0) {
            return (Long) result[0];
        }
     ...
     *
     * @param root
     *            the root object that constrains query execution to only objects reachable (under transitive closure of
     *            reference fields) from this root. This enables localized queries and improves performance by focusing
     *            traversal on a subgraph of the entire object heap.
     * @param cQuery
     *            the Cypher query string to execute on the object graph. The query treats the object heap as a graph
     *            database where each object is a node (labeled with its fully qualified class name), primitive/String
     *            fields are node properties, and reference fields are relationships (labeled with the field name)
     *            between nodes.
     *
     ...
     *
     * <p>
     * Unlike bounded queries, unbounded queries execute on the complete available object heap without restriction to a
     * specific subgraph. This provides access to all objects in memory but with less precise semantics due to the
     * dynamic nature of the JVM and garbage collection.
     *
     * @param cQuery
     *            the Cypher query string to execute on the object graph. The query treats the object heap as a graph
     *            database where each object is a node (labeled with its fully qualified class name), primitive/String
     *            fields are node properties, and reference fields are relationships (labeled with the field name)
     *            between nodes.Supports positional arguments that will be expanded using the provided objects array.
     * @param objects
     *            variadic arguments used to expand positional arguments in the query string:
     *            <ul>
     ...
     *
     * <p>
     * This is the most flexible query method, combining both root-bounded execution and positional argument expansion.
     * The query is constrained to objects reachable from the root object, and the query string can reference runtime
     * values through positional arguments.
     *
     * <p>
     * <b>Example usage:</b>
     *
     ...
     * Object[] result = query(bTree.root, "MATCH (n {$1})-[:left|right*2]->(m:@1 {value:1}) RETURN m", bTree.root,
     *         Node.class);
     * }</pre>
     *
     * @param root
     *            the root object that constrains query execution to only objects reachable (under transitive closure of
     *            reference fields) from this root. This enables localized queries and improves performance by focusing
     *            traversal on a subgraph of the entire object heap. Can be a collection containing multiple roots.
     * @param cQuery
     *            the Cypher query string to execute on the object graph. The query treats the object heap as a graph
     *            database where each object is a node (labeled with its fully qualified class name), primitive/String
     *            fields are node properties, and reference fields are relationships (labeled with the field name)
     ...
     *             if the query execution fails
     */
    public static Object[] query(Object root, String cQuery, Object... objects) throws RemoteException {
        ...
        graphException = new GraphTriggerException(inMemory, forceGC, followRoot, whitelist, cQuery, root,
                whiteList.toArray(), blackList.toArray(), excludeBlackListing.toArray());

        /*
         * Create Object graph Setup Database Create nodes and relations
         */
        setupGraph();

        /*
         * Execute Cypher query Every Query must return a value If query suceeds, value is returned and
         * Cyphssert.assertRelation returns true
         */
        try {
            if (!inMemory) {
            ...
            /*
             * Clear and delete database Delete all *.csv files
             */
            cleanup();
        }
     ...
     *
     * <p>
     * The blacklist optimization helps reduce the size of the object graph by filtering out instances of classes from
     * specified packages. This improves performance by limiting the number of objects traversed and serialized during
     * query execution. Clears any existing blacklist entries before adding the new packages.
     *
     * @param packageNames
     *            package names to exclude from the object graph (e.g., "java.lang", "sun.misc")
     *
     ...
     *
     * <p>
     * Instances of classes belonging to this package will be excluded from the object graph during query execution.
     *
     * @param packageName
     *            the package name to add to the blacklist
     *
     ...
     *
     * <p>
     * After calling this method, no packages will be blacklisted and all instances will be considered for inclusion in
     * the object graph (subject to whitelist constraints).
     *
     * @see #setBlackList(String...)
     */
     ...
     *
     * <p>
     * The whitelist optimization (WL) limits the object graph to only instances of classes from specified packages (and
     * objects reachable from them under transitive closure). This dramatically reduces object graph size and improves
     * query performance. According to the paper, +WL+FGC provides the biggest performance reduction. Clears any
     * existing whitelist entries before adding the new packages.
     *
     * @param packageNames
     *            package names to include in the object graph (e.g., "org.myapp", "com.example")
     *
     ...
     *
     * <p>
     * Instances of classes belonging to this package will be included in the object graph during query execution.
     *
     * @param packageName
     *            the package name to add to the whitelist
     *
     ...
     *
     * <p>
     * After calling this method, all packages may be included in the object graph (subject to blacklist constraints and
     * other filters).
     *
     * @see #setWhiteList(String...)
     */
     ...
     *
     * <p>
     * Even if a package would normally be blacklisted by internal heuristics or other mechanisms, packages in this list
     * will be protected from blacklisting. This provides fine-grained control over which packages are guaranteed to be
     * included. Clears any existing entries before adding the new packages.
     *
     * @param packageNames
     *            package names to exclude from blacklisting
     *
     ...
     *
     * <p>
     * This package will be protected from automatic blacklisting.
     *
     * @param packageName
     *            the package name to exclude from blacklisting
     *
     ...
     *
     * <p>
     * After calling this method, no packages have special protection from blacklisting.
     *
     * @see #setExcludeFromBlackList(String...)
     */
