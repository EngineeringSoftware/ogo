 ...
 * @author 1sand0s
 */
public class Neo4JGraphQueryEngine extends AbstractGraphQueryEngine implements AbstractGraphQueryInterface {
    ...
    /**
     * Use HashMap to create relationships manually between nodes (Faster than Cypher Query approach of CREATE RELATION)
     */
     ...
     *
     * <p>
     * To maintain consistency, I have named relationship types based on JVMTI heap reference kind
     *
     * <p>
     * See <a href="https://docs.oracle.com/en/java/javase/11/docs/specs/jvmti.html#JVMTI_HEAP_REFERENCE_FIELD">...</a>
     */
    ...
    protected final REFERENCE_KIND[] relationArray = { REFERENCE_KIND.JVMTI_HEAP_REFERENCE_CLASS,
            REFERENCE_KIND.JVMTI_HEAP_REFERENCE_FIELD, REFERENCE_KIND.JVMTI_HEAP_REFERENCE_ARRAY_ELEMENT,
            REFERENCE_KIND.JVMTI_HEAP_REFERENCE_CLASS_LOADER, REFERENCE_KIND.JVMTI_HEAP_REFERENCE_SIGNERS,
            REFERENCE_KIND.JVMTI_HEAP_REFERENCE_PROTECTION_DOMAIN, REFERENCE_KIND.JVMTI_HEAP_REFERENCE_INTERFACE,
            REFERENCE_KIND.JVMTI_HEAP_REFERENCE_STATIC_FIELD, REFERENCE_KIND.JVMTI_HEAP_REFERENCE_CONSTANT_POOL,
    ...
    /**
     * @brief Reads files named '[TAG]_Neo4JRelations.csv' containing tags of instances and primitives owned by instance
     *        with tag [TAG]
     *
     * @param file
     *            [TAG]_Neo4JRelations.csv
     * @param refNodes
     *            Tags of owned instances and primitives contained inside the file
     * @param relationType
     *            type of relation interms of JVMTI_HEAP_REFERENCE_KIND
     * @param relationName
    ...
    void readRelationFile(File file, ArrayList<Long> refNodes, ArrayList<REFERENCE_KIND> relationType,
        ...
        try {
            ...
            while (scan.hasNext()) {
                ...
                /*
                 * The number of tokens for each line read must be 2, else throw IOException + TODO : Create custom
                 * Exception
                 */
                ...
                /*
                 * Second column in [*TAG*].Neo4JRelations.csv is the reference kind between the referrer and referee
                 * object
                 */
                relationType.add(relationArray[Integer.parseInt(tokens.nextToken()) - 1]);

                /*
                 * Third column in [*TAG*].Neo4JRelations.csv is the name of the field corresponding to the referee
                 * object as declared in the Class of the referrer object
                 */
    ...
    public void createNodes() throws RemoteException {
        ...
        for (File file : files) {
            ...
            String cQuery = "LOAD CSV WITH HEADERS FROM 'file://" + file.getAbsolutePath() + "' As row\n"
                    + "CREATE (n {signature:row.Signature, tag:toInteger(row.Tag) ,"
                    + " hash:toInteger(row.HashCode)})\n" + "RETURN" + " ID(n)" + " , n.tag" + " , n.signature"
                    + " , n;"; // Return ID and Tag to store in HashMap
            // for creating relations later
            try (Transaction tx = graphDb.beginTx(); Result result = tx.execute(cQuery)) {
                while (result.hasNext()) {
                    ...
                    for (int k = 0; k < propertyName.size(); k++) {
                        ...
                        CastObjectArrayToPrimitive.castToNeo4JPropertyType(propertyName.get(k), propertyValue.get(k),
                                propertyType.get(k), node);
                        /*
                         * if(propertyValue.get(k).length == 1) node.setProperty(propertyName.get(k),
                         * propertyValue.get(k)[0]); else node.setProperty(propertyName.get(k), propertyValue.get(k));
                         */
                    }
                }
    ...
    public void createRelations() throws RemoteException {
        ...
        /*
         * Get all [*TAG*]_Neo4JRelation.csv files in current directory describing relations between instances and
         * between instances and primitives
         */
        ...
        for (File file : files) {
            try (Transaction tx = graphDb.beginTx()) {
                ...
                Long referrerTag = Long.parseLong((file.getName()).substring(0, (file.getName()).indexOf("_")));

                /*
                 * Read [*REFERRER TAG*]_Neo4JRelations.csv file to get 'referee tags', reference kinds and reference
                 * names
                 */
                ...
                if (referrerNodeID != null) {
                    ...
                    for (int k = 0; k < refereeTags.size(); k++) {
                        ...
                        if (refereeNodeID != null) {
                            ...
                            /*
                             * Create relation between 'referrer node' and 'referee node' with name of the field
                             * corresponding to the referee instance as declared in the class of referrer instance
                             */
                            Relationship relation = referrerNode.createRelationshipTo(tx.getNodeById(refereeNodeID),
                                    RelationshipType.withName(relationName.get(k)));

                            /*
                             * Assign reference type See :
     ...
     *
     * @param cQuery
     *            String in Cypher to query the database
     *
    ...
    public ArrayList<Object> query(String cQuery) throws RemoteException {
        ...
        /*
         * Return status fo query True if query succeeded (Cypher query must have RETURN command)
         */
        return res;
    }
    ...
    public static void main(String[] args) throws IOException, AlreadyBoundException {
        ...
        AbstractGraphQueryInterface stub = (AbstractGraphQueryInterface) UnicastRemoteObject.exportObject(engine, 0);
