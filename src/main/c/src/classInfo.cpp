...
MethodInfo* ClassInfo::getMethodMatchingDescriptor(string &methodName,
 ...
 * @param   iInfo       populated with the instance bearing the queried tag if found
 */
void ClassInfo::getInstanceInfoWithTag(long tag,
        InstanceInfo **iInfo) {
    int index = -1;

    /*Check if tag was found before retrieving corressponding instanceInfo*/
    if (UTIL::AGENT::searchTag(0, instances.size() - 1, tag, instanceTags, &index)) {
 ...
 */
void
ClassInfo::sortElementAscendingTag() {
