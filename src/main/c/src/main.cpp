 ...
 * of function call returns. If no error then jvmtiError must be JVMTI_ERROR_NONE
 * see:
 *      1. https://docs.oracle.com/en/java/javase/11/docs/specs/jvmti.html#functionReturn
 *      2. https://docs.oracle.com/en/java/javase/11/docs/specs/jvmti.html#ErrorSection
 *      3. https://docs.oracle.com/en/java/javase/11/docs/specs/jvmti.html#jvmtiError
 */
...
/**
 *  Number of classes in the classNameList (Not all loaded classes are stored in classNameList
 *  Only those used in writing assertions)
 *
 ...
 * belonging to these packages will be definitely excluded from the object classNameList
 */
 ...
 */
static void
getObjectsWithTagscInfo(jobject** objects,
                        long** tags,
                        int* tagCount,
                        TIMER::TimeInfo* tInfo) {
    long* cTags = NULL;
    int cTagCount = 0;
    *objects = NULL;
    *tags = NULL;
    *tagCount = 0;
    ...
    for(ClassInfo* cInfo : classNameList) {
        if (cInfo->instances.size() == 0 ||
                !cInfo->containsWritableInstances() ||
                cInfo->name.size() == 0) {
            continue;
        }
    ...
    TIMER_START_(*tInfo)
    agent->getObjectsWithTags(cTags,
                              cTagCount,
                              objects,
                              tags,
                              tagCount);
 ...
 */
static void
writeClassNameListToCsv(TIMER::TimeInfo** tInfo) {

    jobject* objects = NULL;
    long* tags = NULL;
    int tagCount = 0;

    getObjectsWithTagscInfo(&objects,
                            &tags,
                            &tagCount,
                            *tInfo);

    for (ClassInfo* cInfo : classNameList) {
        if (!cInfo->containsWritableInstances() ||
                cInfo->name.size() == 0) {
            continue;
        }
        ...
        for (int i = 0; i < tagCount; i++) {
            ...
            cInfo->getInstanceInfoWithTag(tags[i],
                                          &info);
...
/**
 * @fn          static jint JNICALL heap_iterationCallback(jlong, jlong, jlong*, jint, void*)
 * @brief       Search for class instances and assign unique tag to them. The tag
 *              can later be used by followReferences to identify relationships. Unique
 *              tag only assigned to instance types whereas primtives, arrays and string types
 *              are by default assigned the same tag as the instance they belong to
 *
 * @author      1sand0s
 * @param       class_tag   Tag of the class of the current instance
 * @param       size        Not used
 * @param       tag_ptr     Tag of the current instance (0 unless instance of java.lang.Class)
 *                          Assigned to unique tag to be used later during follow references
 * @param       length      Not used
 * @param       user_data   Not used
 *
 */
jint JNICALL
heap_iterationCallback(jlong class_tag,
                       jlong size,
                       jlong *tag_ptr,
                       jint length,
                       void *user_data) {
    if (class_tag > 0 && class_tag <= classInfoCount) {
        if (
             ...
             * 2. We need this object since all instances of a Class hold a reference (instanceof)
             *    to their Class's Class.java object
             * 3. We also use this object for getting the Class's meta-data such as fields and methods
             */
            (*tag_ptr > 0) ||

            /* 1. This checks if current object is an instance of a Class which was tagged in
             *    getLoadedClasses(). We use this check to limit the size of the Object classNameList.
             * 2. Without this check all objects present in the JVM's heap would be loaded into
             *    the object classNameList
             * 3. We need to filter out Class.Class.java objects since even untagged classes will
             *    have Class.Class.java objects
             */
            (*tag_ptr == 0)) {
            ...
            /* If its untagged object then its an instance of a Class rather than Class's Class.java
             * and so assign a unique tag to identify relations between instances during
             * followReferences
             *
             * The monotonously increasing tag assigned to objects here assures that all objects inserted
             * into the respective class's classInfo struct happens such that their tags are in ascending
             * order. So, we don't have to do any extra processing
             */
            if (*tag_ptr == 0) {
                *tag_ptr = ++nodeID;
            }
            /* If the object is tagged then its an instance of Class.Class.java and therefore the callback
             * can report this instance in any order (not necessarily ascending order of tags). But since we
             * use binary search to get instances of classes using their tags, we want the instance array to be
             * sorted in ascending order of tags. So set this classInfo element unsorted (Mergesort used later
            ...
            /* Store instance tags in instanceTags of classInfo(makes searching for instances with
             * specified tags faster)*/
...
/**
 * @fn          static jint JNICALL heap_iterationSetupCallback(jlong, jlong, jlong*, jint, void*)
 * @brief       This sets the tag of all objects in the heap to 0. This serves as an intialization step
 *              before we construct the object classNameList
 *
 * @author      1sand0s
 * @param       class_tag   Tag of the class of the current instance
 * @param       size        Not used
 ...
 */
jint JNICALL
heap_iterationSetupCallback(jlong class_tag,
                            jlong size,
                            jlong *tag_ptr,
                            jint length,
                            void *user_data) {
    /* For cleaning up and initialization, re-initialize all object tags back to 0*/
    *tag_ptr = 0;
    return JVMTI_VISIT_OBJECTS;
}
 ...
 *                                                  jint,
 *                                                  void*)
 * @brief       We use the unique tags assigned to instances inside heap_iterationCallback
 *              to identify relations between instances and between instances and their primitives
 *
 * @author      1sand0s
 * @param       reference_kind
 * @param       reference_info
 * @param       class_tag               Tag of the class of the current instance
 * @param       referrer_class_tag      Tag of the class whose instance owns the current instance
 * @param       size                    Not used
 * @param       tag_ptr                 Tag of the current instance
 * @param       referrer_tag_ptr        Tag of the instance who owns the current instance
 * @param       length                  Not used
 * @param       user_data               Not used
 */
jint JNICALL
heap_referenceCallback(jvmtiHeapReferenceKind reference_kind,
                       ...
                       void *user_data) {
    if (
        /* Classes of referrer instances limited to tagged class (Will Segfault else) */
        referrer_class_tag > 0 && referrer_class_tag <= classInfoCount &&

        /* Classes of referee instances limited to tagged classes to control object classNameList size */
	  ...
	  //return JVMTI_VISIT_ABORT;
        ...
        /*if(classNameList[class_tag - 1]->name.find("org.ogo.test.korat.BinaryTree") != string::npos ||
                classNameList[referrer_class_tag - 1]->name.find("org.ogo.test.korat.BinaryTree") != string::npos
				) {*/
            cout << "Object Tag : " << *tag_ptr << " , Referrer Tag : " << *referrer_tag_ptr << " , Class Tag : " << classNameList[class_tag - 1]->name << "  " << class_tag << " , Referrer Class Tag : " << classNameList[referrer_class_tag - 1]->name << "  " << referrer_class_tag << " , Heap Reference Kind : " << reference_kind << endl;
			//}
#endif
        ...
        /* Get instanceInfo with tag matching referrer_tag_ptr */
GETINSTANCE:
        classNameList[referrer_class_tag - 1]->getInstanceInfoWithTag(*referrer_tag_ptr,
                &iInfo);
#ifdef OPTIMIZATION_CACHE_RETRIEVED_INSTANCE_INFO_
    }
#endif
    ...
    if (iInfo != NULL) {
        ...
        ReferenceInfo* rInfo = new ReferenceInfo(reference_kind,
                *tag_ptr,
                class_tag,
                iInfo->writeToGraph);


        /* If the current instance is a reference field of the referrer instance then
             * store its fieldIndex which can be used to get the name of the current instance
             * from jfieldID (see getFieldInfo())
             * *ONLY FOR INSTANCES OF TYPE java.lang.String or Primitive Boxed types*
             */
		   ...
		   reference_kind == JVMTI_HEAP_REFERENCE_INTERFACE) {
            if(
			  ...
			  if(reference_kind == JVMTI_HEAP_REFERENCE_SUPERCLASS)
				...
				classNameList[*referrer_tag_ptr - 1]->implementedInterfaceTags.push_back(*tag_ptr);
            }
        }
 ...
 *                                                       jvmtiPrimitiveType,
 *                                                       void*)
 * @brief       Get primitive fields of an instance during follow references. Used to get values
 *              of th primitive fields.
 *
 * @author      1sand0s
 * @param       reference_kind      Enum describing the type of reference between owning instance and primitive field
 * @param       reference_info
 * @param       object_class_tag    Tag of the class of the instance who owns the current primitive field
 * @param       object_tag_ptr      Tag of the primitive field (by default equal to tag of the owning instance)
 * @param       value               value of the primitive field
 * @param       value_type          type of the primitive field (used to get appropriate field from union value)
 * @param       user_data           Not used
 */
jint JNICALL
heap_primitiveFieldCallback(jvmtiHeapReferenceKind kind,
    ...
    if (
        ...
        *object_tag_ptr > 0) {
#ifdef  DEBUG_PRINT_HEAP_TRAVERSAL_
        cout << "Field : Class : " << object_class_tag << " , Tag : " << *object_tag_ptr << " , Index : " << info->field.index << " , Reference Kind : " << kind << " , Field Tag : " << *object_tag_ptr << endl;
...
GETINSTANCE:
    ...
    if (iInfo != NULL) {
        ...
        /* See https://docs.oracle.com/en/java/javase/11/docs/specs/jvmti.html#jvmtiHeapReferenceCallback
         * Can only be JVMTI_HEAP_REFERENCE_FIELD (2) or JVMTI_HEAP_REFERENCE_STATIC_FIELD (8) */
 ...
 *
 * @author      1sand0s
 * @param       class_tag    Tag of the class of the instance who owns the current string field
 * @param       tag_ptr      Tag of the string field (by default equal to tag of the owning instance)
 * @param       value               value of the string field
 * @param       value_length  Length of the string
 * @param       user_data           Not used
 */
jint JNICALL
heap_stringFieldCallback(jlong class_tag,
                         jlong size,
                         jlong* tag_ptr,
                         const jchar* value,
                         jint value_length,
    ...
    if (
        ...
        *tag_ptr > 0) {
#ifdef  DEBUG_PRINT_HEAP_TRAVERSAL_
        cout << "String Field : Class Tag : " << class_tag << " , Tag : " << *tag_ptr << " , Value : ";
        ...
        /* Get instanceInfo with tag matching object_tag_ptr */
GETINSTANCE:
        classNameList[class_tag - 1]->getInstanceInfoWithTag(*tag_ptr,
                &iInfo);
#ifdef OPTIMIZATION_CACHE_RETRIEVED_INSTANCE_INFO_
    }
#endif
 ...
 */
jint JNICALL
heap_arrayFieldCallback(jlong class_tag,
                        jlong size,
                        jlong* tag_ptr,
                        jint element_count,
    ...
    if (
        ...
        *tag_ptr > 0) {
#ifdef  DEBUG_PRINT_HEAP_TRAVERSAL_
        cout << "Array Field : Class Tag : " << class_tag << " , Tag : " << *tag_ptr << " Value : ";
        for(int j = 0; j<element_count; j++) {
            switch(element_type) {
#define ENTRY(a, b, c, d, e, f, g) case a: printf(STRINGIFY_MACRO_(b), *((( d * )elements) + j)); break;
...
#define ENTRY(a, b, c, d) case Agent::MethodReturnType::c: fInfo->type = Agent::MethodReturnType::c##_ARRAY; for(int j = 0; j < element_count; j++) { jvalue value{. d = ((b*) elements)[j]}; fInfo->value.push_back(value);} break;
...
/**
 * @fn void embedStringInstances(const instanceInfo*, instanceInfo*, const int, classInfo*)
 * @brief
 *
 ...
 */
static void
embedStringInstances(const InstanceInfo* stringInstance,
    ...
    for(int j = 0; j < stringInstance->references.size(); j++) {
        ...
        if(stringInstance->references[j]->referenceKind == JVMTI_HEAP_REFERENCE_FIELD) {

            InstanceInfo* byteArrayInstance = NULL;

            classNameList[stringInstance->references[j]->referrerClassTag - 1]->getInstanceInfoWithTag(stringInstance->references[j]->referrerTag,
                    &byteArrayInstance);
...
/**
 * @fn void embedBoxedPrimitiveInstances(const instanceInfo*, instanceInfo*, const int, classInfo*)
 * @brief
 *
 ...
 */
static void
assignFieldNamesToReferenceFields(InstanceInfo* referrer,
                                  int referenceIndex,
    ...
    if((refereeFieldIndex < 0 &&
            ...
            /* String Array elements will have fieldIndex < 0 but still need to be embedded */
		referrer->references[referenceIndex]->referenceKind != JVMTI_HEAP_REFERENCE_ARRAY_ELEMENT)||
    ...
    classNameList[referrer->references[referenceIndex]->referrerClassTag - 1]->getInstanceInfoWithTag(referrer->references[referenceIndex]->referrerTag,
            &referee);
	  ...
	  //else
		{
		  ...
		  referrer->references[referenceIndex]->referrerRelation = inheritedFields[refereeFieldIndex]->name;
        }
    }
}

void getImplementedInterfaceAndSuperInterfaces(std::vector<long>& implementedInterfaceTags,
  ...
  for(long interfaceTag : implementedInterfaceTags){
    ...
    getImplementedInterfaceAndSuperInterfaces(interface->implementedInterfaceTags,
											  allImplementedInterfaceTags);
	allImplementedInterfaceTags.insert(interface->tag);
  }
}

void getAllImplementedInterfaces(ClassInfo* cInfo,
								 unordered_set<long>& allImplementedInterfaceTags){
  if(cInfo == NULL)
	return;
...
assignFieldNames() {
    for (int i = 0; i < classInfoCount; i++) {
        ...
        for (int j = 0; j < cInfo->instances.size(); j++) {
            ...
            if(referrer->isPrimitiveArrayType ||
                    referrer->isStringType) {
                continue;
            }
            ...
            int fieldCount = 0;
			unordered_set<long> allImplementedInterfaceTags;
			getAllImplementedInterfaces(classInstance,
						    allImplementedInterfaceTags);
            ...
            getInheritedFieldInfo(classInstance,
                                  inheritedFields,
				  fieldCount);
            ...
            for (FieldInfo* referrerField : referrer->fields) {
			  ...
			  if (inheritedFields.count(referrerField->fieldIndex) == 1) {
                    referrerField->name = inheritedFields[referrerField->fieldIndex]->name;
                    referrerField->signature = inheritedFields[referrerField->fieldIndex]->signature;
                }
                /* We only add name, the signature (Type) and modifier must be accessed through
                 * the field's owning instance's Class.java object */
            }

            for (int k = 0; k < referrer->references.size(); k++) {

                if(referrer->references[k]->referenceKind == JVMTI_HEAP_REFERENCE_FIELD ||
                        referrer->references[k]->referenceKind == JVMTI_HEAP_REFERENCE_STATIC_FIELD ||
                        referrer->references[k]->referenceKind == JVMTI_HEAP_REFERENCE_ARRAY_ELEMENT) {
                    assignFieldNamesToReferenceFields(referrer,
                                                      k,
                                                      inheritedFields);
                } else if(referrer->references[k]->referenceKind == JVMTI_HEAP_REFERENCE_CLASS) {
 ...
 * @brief       Get all fields declared in the class and inherited by the class (SHALLOW COPY)
 *
 * @author      1sand0s
 * @param       cInfo       The class whose declared and inehrited fields have to be retrieved
 * @param       fields      Contains the declared and inherited fields
 * @param       fieldCount  Number of elements in fields
 */
static void
getInheritedFieldInfo(const ClassInfo* cInfo,
    ...
    /*for(int j = 0; j < cInfo->classClassInstance->references.size(); j++) {
        if(cInfo->classClassInstance->references[j]->referenceKind == JVMTI_HEAP_REFERENCE_SUPERCLASS &&
		   (cInfo->classClassInstance->references[j]->referrerTag - 1) < classInfoCount) {
            ClassInfo* superClass = classNameList[cInfo->classClassInstance->references[j]->referrerTag - 1];
 ...
 */
static void
assignWriteToClassNameListToReferences(InstanceInfo* iInfo) {
    if(iInfo == NULL) {
        return;
    }
    ...
    for (int k = 0; k < iInfo->references.size(); k++) {
         ...
         * and superclasses will be written to classNameList for Class.Class.java instance*/
 ...
 */
static void
assignWriteToClassNameListToReferences() {
    for (int j = 0; j < classInfoCount; j++) {
        for (int i = 0; i < classNameList[j]->instances.size(); i++) {
            ...
            for (int k = 0; k < classNameList[j]->instances[i]->references.size(); k++) {

                int referenceKind = classNameList[j]->instances[i]->references[k]->referenceKind;
                ...
                long refClass = classNameList[j]->instances[i]->references[k]->referrerClassTag;
                long refTag = classNameList[j]->instances[i]->references[k]->referrerTag;

                InstanceInfo *info = NULL;

                /* Get the instance corresponding to the reference */
                classNameList[refClass - 1]->getInstanceInfoWithTag(refTag,
                        &info);
      ...
      for (int j = 0; j < classInfoCount; j++) {
          if (classNameList[j].tag > 0) {
              ...
              for (int k = 0; k < classNameList[j].instanceCount; k++) {
                  printf("Object%d Tag : %ld , Write To classNameList : %d , Array Type : %d\n",
                         k,
                         classNameList[j].instances[k].name == NULL ? "" : classNameList[j].instances[k].name,
                         *classNameList[j].instances[k].signature,
                  ...
                  for (int i = 0; i < classNameList[j].instances[k].fieldCount; i++) {
                      printf("\tPrimitive Fields%d Name :  , Signature :  , Modifier :  , Tag : %ld\n",
                             i,
                             classNameList[j].instances[k].fields[i].name == NULL ? "" : classNameList[j].instances[k].fields[i].name,
                             *classNameList[j].instances[k].fields[i].signature == NULL ? "" : classNameList[j].instances[k].fields[i].signature,
                              *classNameList[j].instances[k].fields[i].modifier,

                             classNameList[j].instances[k].fields[i].tag);
                      if(classNameList[j].instances[k].fields[i].type != Agent::JVMTI_EXTENDED_FIELD_TYPE_STRING) {
                          for(int m = 0; m < classNameList[j].instances[k].fields[i].valueCount; m++) {
                              printPrimitiveFieldValue2(classNameList[j].instances[k].fields[i].type, classNameList[j].instances[k].fields[i].value.primitive[m]);
                          }
                          printf("\n");
                      }
                  }
                  for (int i = 0; i < classNameList[j].instances[k].referenceCount; i++) {
                      printf("\tReference Fields%d Tag : %ld , Class Tag : %ld\n",
                             i,
 ...
 * @brief       Get information regarding all loaded classes in the JVM and store
 *              in classNameList. Unique tags are also assigned to Class.java objects
 *
 * @author      1sand0s
 */
static void
getLoadedClasses() {
    ...
    for (jclass klass : classes) {
        ...
        /* Assign a tag to Class.java object for associating objects during heap traversal
         * 1. Every instance of a Class holds a reference to its Class's Class.java object
         * 2. We tag the Class.java object here so we can use this reference/relation to identify
         *    the instance's class without additional JNI calls (expensive).
         */
        ...
        /* Get class fields */
        agent->getClassDeclaredFields(klass,
                                      fields);
...
#ifdef DEBUG_PRINT_
        if(STRING::isStringPresentInList(className, whitePackageName)) {
            cout << "\nClass Name : " << cInfo->name << " , Class Signature : " << cInfo->signature << " , Class Tag : " << cInfo->tag << " , Write To ClassNameList : " << cInfo->writeToGraph << "\n";

            /* Print all class fields */
            for (FieldInfo* fInfo : cInfo->fields) {
                cout << "Field Name : " << fInfo->name << " , Signature : " << fInfo->signature << " , Modifier : " << fInfo->modifier << "\n";
            }
            /* Print all class fields */
			/*  for (MethodInfo* mInfo : cInfo->methods) {
                cout << "Method Name : " << mInfo->name << " , Signature : " << mInfo->signature << " , Modifier : " << mInfo->modifier << "\n";
				}*/
...
setup() {
    ...
    */
    ...
    classNameList.erase(classNameList.begin(), classNameList.begin() + classNameList.size());
 ...
 *                                            jmethodID,
 *                                            jlocation)
 * @brief       Use dummy exception to trigger object classNameList construction. Much more efficient
 *              than using method_entry events
 *
 * @author      1sand0s
 ...
 * @param       method          The java method in which the exception was thrown
 * @param       location
 * @param       exception       The type of exception
 * @param       catch_method    The java method in which the exceptionw as caught
 * @param       catch_location
 */
void JNICALL
exception_thrown(jvmtiEnv *jvmti_env,
                 JNIEnv *jni_env,
                 jthread thread,
                 jmethodID method,
                 jlocation location,
    ...
    TIMER::TimeInfo total_,
          ...
          getLoadedClasses_,
          iterateHeap_Assign_,
          sortElementAscendingTag_,
          followReferences_,
          assignSuperClass_,
          assignFieldNames_,
          assignWriteToClassNameListToReferences_,
          writeClassNameListToCsv_,
          *writeClassNameListToCsv_Internal_;
    /* Get name of method throwing exception */

    /* If exception is thrown from method "setupClassNameList" then proceed with classNameList construction */
    if (agent->getMethodName(method) == OGO::GRAPH_EXCEPTION_TRIGGER_METHOD_NAME) {
        agent->setJNIEnv(jni_env);
#ifdef DEBUG_PRINT_EXCEPTION_CLASS_
        cout << "Debug : Exception Class Name : " << agent->getClassName(exception) << "\n";
#endif
        TIMER_START_ (total_)
        inMemory = agent->evaluateBooleanObject(agent->getInstanceObjectField(exception,
                                                OGO::CYPHER_QUERY_IN_MEMORY_FIELD,
                                                "Ljava/lang/Boolean;"));
		forceGC = agent->evaluateBooleanObject(agent->getInstanceObjectField(exception,
                                                OGO::CYPHER_QUERY_FORCE_GC_FIELD,
                                                "Ljava/lang/Boolean;"));
		followRoot = agent->evaluateBooleanObject(agent->getInstanceObjectField(exception,
                                                OGO::CYPHER_QUERY_FOLLOW_ROOT_FIELD,
                                                "Ljava/lang/Boolean;"));
		whiteList = agent->evaluateBooleanObject(agent->getInstanceObjectField(exception,
                                                OGO::CYPHER_QUERY_WHITELIST_FIELD,
                                                "Ljava/lang/Boolean;"));
        //cout << "In memory " << inMemory << endl;
        rootObject = agent->getInstanceObjectField(exception,
                     OGO::CYPHER_QUERY_ROOT_OBJECT_FIELD,
                     "Ljava/lang/Object;");
        TIMER_START_ (setup_)
        ...
        TIMER_END_ (setup_)
        if(inMemory) {
            cQuery = agent->evaluateStringObject((jstring) agent->getInstanceObjectField(exception,
                                                 OGO::CYPHER_QUERY_STRING_FIELD,
                                                 "Ljava/lang/String;"));
        ...
        /* Read Whitelist of Packages */
		  agent->getInstanceStringArrayField(exception, OGO::WHITE_LIST_FIELD, "[Ljava/lang/Object;", whitePackageName);
		}
        ...
        agent->getInstanceStringArrayField(exception, OGO::BLACK_LIST_FIELD, "[Ljava/lang/Object;", blackPackageName);
		agent->getInstanceStringArrayField(exception, OGO::EXCLUDE_FROM_BLACK_LIST_FIELD, "[Ljava/lang/Object;", excludeblackPackageName);
         ...
         * (Initialization Step) */
        agent->iterateThroughHeap(&iterationSetupCallbacks, JVMTI_HEAP_FILTER_UNTAGGED);
        ...
        agent->iterateThroughHeap(&iterationCallbacks, JVMTI_HEAP_FILTER_CLASS_UNTAGGED);
        ...
        agent->followReferences(&traversalCallbacks, JVMTI_HEAP_FILTER_UNTAGGED, rootObject);
        ...
        if(inMemory) {
            computeQuery(cQuery,
                         results);
            if(results.size() > 0) {
                agent->setObjectReferenceField(exception,
                                               agent->createObjectArray(results),
                                               OGO::CYPHER_QUERY_RESULT_FIELD,
                                               "[Ljava/lang/Object;");
            }
        } else {
            writeClassNameListToCsv_Internal_ = (TIMER::TimeInfo*) calloc(6, sizeof(struct TIMER::TimeInfo));
        ...
        timenow = gmtime(&now);
        strftime(profileFile, sizeof(profileFile), "PROFILE_HOQ_C_%Y-%m-%d_%H:%M:%S", timenow);
        pFile = fopen(profileFile,"w");
        if(pFile == NULL) {
            printf("Failed to create file for writing profile data %s\n", profileFile);
            exit(1);
        }
        fprintf(pFile, "Setup, %f\n"
                "Forcing_GC, %f\n"
                ...
                DURATION_(setup_),
                DURATION_(forceGC_),
                DURATION_(iterateHeap_Init_),
                DURATION_(getLoadedClasses_),
                DURATION_(iterateHeap_Assign_),
                DURATION_(sortElementAscendingTag_),
                DURATION_(followReferences_),
                DURATION_(assignSuperClass_),
                DURATION_(assignFieldNames_),
                DURATION_(writeClassNameListToCsv_),
                classInfoCount,
                nodeID,
				objectWrittenToClassNameListCount,
                DURATION_(total_));
...
void acceptVisitor(string query,
...
void setupCallbacks() {
    ...
    (void) memset(&iterationCallbacks,
                  0,
                  sizeof(iterationCallbacks));
    iterationCallbacks.heap_iteration_callback = &heap_iterationCallback;

    /* Allocate memory for struct defining callback functions */
    (void) memset(&iterationSetupCallbacks,
                  0,
                  sizeof(iterationSetupCallbacks));
    iterationSetupCallbacks.heap_iteration_callback =
        &heap_iterationSetupCallback;

    /* Allocate memory for struct defining callback functions */
    (void) memset(&traversalCallbacks,
                  0,
                  sizeof(traversalCallbacks));
    ...
    traversalCallbacks.string_primitive_value_callback = &heap_stringFieldCallback;
...
Agent_OnUnload(JavaVM *vm) {
    /* No point freeing memory since VM is killed anyway, but doing it for good practice */
    setup();
}
