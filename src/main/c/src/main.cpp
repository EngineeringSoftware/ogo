    ...
    *tagCount = 0;
    ...
    for (ClassInfo *cInfo : classNameList) {
        ...
        for (int k = 0; k < cInfo->instances.size(); k++) {
            ...
            cTags = static_cast<long *>(
                    realloc(cTags, (cTagCount + 1) * sizeof(long)));
...
jint JNICALL heap_iterationCallback(jlong class_tag, jlong size, jlong *tag_ptr,
    ...
    if (class_tag > 0 && class_tag <= classInfoCount) {
        if (
                /* 1. This checks if current object is an instance of
                 * Class.Class.java
                 * 2. We need this object since all instances of a Class hold a
                 * reference (instanceof) to their Class's Class.java object
                 * 3. We also use this object for getting the Class's meta-data
                 * such as fields and methods
                 */
                (*tag_ptr > 0) ||

                /* 1. This checks if current object is an instance of a Class
                 * which was tagged in getLoadedClasses(). We use this check to
                 * limit the size of the Object classNameList.
                 * 2. Without this check all objects present in the JVM's heap
                 * would be loaded into the object classNameList
                 * 3. We need to filter out Class.Class.java objects since even
                 * untagged classes will have Class.Class.java objects
                 */
                (*tag_ptr == 0)) {
            ...
            /* If its untagged object then its an instance of a Class rather
             * than Class's Class.java and so assign a unique tag to identify
             * relations between instances during followReferences
             *
             * The monotonously increasing tag assigned to objects here assures
             * that all objects inserted into the respective class's classInfo
             * struct happens such that their tags are in ascending order. So,
             * we don't have to do any extra processing
             */
            if (*tag_ptr == 0) {
                *tag_ptr = ++nodeID;
            }
            /* If the object is tagged then its an instance of Class.Class.java
             * and therefore the callback can report this instance in any order
             * (not necessarily ascending order of tags). But since we use
             * binary search to get instances of classes using their tags, we
             * want the instance array to be sorted in ascending order of tags.
             * So set this classInfo element unsorted (Mergesort used later to
             * sort its instances)
             */
            else {
            ...
            iInfo->writeToGraph =
                    classNameList[classNameListIndex]->writeToGraph;

            /* Store instance tags in instanceTags of classInfo(makes searching
             * for instances with specified tags faster)*/
...
jint JNICALL heap_referenceCallback(
    ...
    if (
            /* Classes of referrer instances limited to tagged class (Will
               Segfault else) */
        ...
        cout << "Object Tag : " << *tag_ptr
             ...
             << referrer_class_tag
             << " , Heap Reference Kind : " << reference_kind << endl;
        //}
#endif
    ...
    if (iInfo != nullptr) {
         ...
         * current instance from jfieldID (see getFieldInfo()) *ONLY FOR
         * INSTANCES OF TYPE java.lang.String or Primitive Boxed types*
         */
        ...
        if (reference_kind == JVMTI_HEAP_REFERENCE_ARRAY_ELEMENT) {
            rInfo->referrerRelation =
                    std::to_string(reference_info->array.index);
        }
         ...
         */
        if (
                /* Check if reference between referrer and referee instance is
                 * of type constant pool
                 */
                reference_kind == JVMTI_HEAP_REFERENCE_SUPERCLASS ||
                reference_kind == JVMTI_HEAP_REFERENCE_INTERFACE) {
            if (
                    ...
                    *referrer_tag_ptr < classInfoCount &&
                    ...
                    /* If both referrer and referee are Class.Class.java
                     * instances then their class's must be the same
                     * (Class.java) and hence check if their class tags are
                     * equal (this condition can perhaps subsume the previous
                     * two?)
                     */
                    class_tag == referrer_class_tag) {

                if (reference_kind == JVMTI_HEAP_REFERENCE_SUPERCLASS)
                    /* Shallow copy the Class.java instance into this class's
                     * classInfo*
                     */
                    classNameList[*referrer_tag_ptr - 1]->superClassTag =
                            *tag_ptr;
...
jint JNICALL heap_primitiveFieldCallback(jvmtiHeapReferenceKind kind,
    ...
    GETINSTANCE:
        classNameList[object_class_tag - 1]->getInstanceInfoWithTag(
                *object_tag_ptr, &iInfo);
#ifdef OPTIMIZATION_CACHE_RETRIEVED_INSTANCE_INFO_
    }
#endif
...
jint JNICALL heap_arrayFieldCallback(jlong class_tag, jlong size,
    ...
    if (
            ...
            *tag_ptr > 0) {
#ifdef DEBUG_PRINT_HEAP_TRAVERSAL_
        cout << "Array Field : Class Tag : " << class_tag
             << " , Tag : " << *tag_ptr << " Value : ";
    ...
    for (const auto reference : stringInstance->references) {
        ...
        if (reference->referenceKind == JVMTI_HEAP_REFERENCE_FIELD) {
            ...
            if (byteArrayInstance != nullptr) {
                /* Since the expected field is of type [B we check if the
                 * retrieved referee instance is of Array Type and has atleast 1
                 * element
                 */
                    ...
                    for (FieldInfo *fInfo : byteArrayInstance->fields) {
                        if (agent->isFieldArrayType(fInfo->type) &&
                            ...
                            if (stringFieldIndex >= 0) {
                                fInfo->name =
                                        inheritedFields[stringFieldIndex]->name;
...
assignFieldNamesToReferenceFields(const InstanceInfo *referrer,
    ...
    classNameList[referrer->references[referenceIndex]->referrerClassTag - 1]
            ->getInstanceInfoWithTag(
                    referrer->references[referenceIndex]->referrerTag,
                    &referee);
...
void getImplementedInterfaceAndSuperInterfaces(
    ...
    for (long interfaceTag : implementedInterfaceTags) {
        ...
        getImplementedInterfaceAndSuperInterfaces(
                interface->implementedInterfaceTags,
...
static void assignFieldNames() {
    for (int i = 0; i < classInfoCount; i++) {
        ...
        for (int j = 0; j < cInfo->instances.size(); j++) {
            ...
            /* We add String and other Primitive Boxed types as properties
             * rather than as their own nodes hence this exception */
            ...
            /* If the tag of the instance is less than classInfoCount then it
             * must be an instance of Class.Class.java and so its fields are
             * declared in Class.Class.java object */
            ...
            for (FieldInfo *referrerField : referrer->fields) {
                ...
                if (inheritedFields.count(referrerField->fieldIndex) == 1) {
                    ...
                    referrerField->signature =
                            inheritedFields[referrerField->fieldIndex]
                                    ->signature;
                }
                /* We only add name, the signature (Type) and modifier must be
                 * accessed through the field's owning instance's Class.java
                 * object */
            }
            ...
            for (int k = 0; k < referrer->references.size(); k++) {
                ...
                if (referrer->references[k]->referenceKind ==
                    ...
                    referrer->references[k]->referenceKind ==
                            JVMTI_HEAP_REFERENCE_ARRAY_ELEMENT) {
                    assignFieldNamesToReferenceFields(referrer, k,
                                                      inheritedFields);
...
static void assignWriteToClassNameListToReferences() {
    for (int j = 0; j < classInfoCount; j++) {
        for (int i = 0; i < classNameList[j]->instances.size(); i++) {
            ...
            for (int k = 0;
                 k < classNameList[j]->instances[i]->references.size(); k++) {

                const int referenceKind = classNameList[j]
                                                  ->instances[i]
                                                  ->references[k]
                                                  ->referenceKind;

                /* Only static fields, references to other Class.Class.java
                 * instances and superclasses will be written to classNameList
                 * for Class.Class.java instance*/
                if (tag <= classInfoCount) {
                    if (referenceKind != JVMTI_HEAP_REFERENCE_FIELD ||
                        referenceKind != JVMTI_HEAP_REFERENCE_CONSTANT_POOL ||
                        referenceKind != JVMTI_HEAP_REFERENCE_SUPERCLASS) {
                ...
                long refClass = classNameList[j]
                                        ->instances[i]
                                        ->references[k]
                                        ->referrerClassTag;
                long refTag = classNameList[j]
                                      ->instances[i]
                                      ->references[k]
                                      ->referrerTag;
...
static void printClassNameList() {
      ...
      for (int j = 0; j < classInfoCount; j++) {
          if (classNameList[j].tag > 0) {
              ...
              for (int k = 0; k < classNameList[j].instanceCount; k++) {
                  printf("Object%d Tag : %ld , Write To classNameList : %d ,
    Array Type : %d\n", k, classNameList[j].instances[k].name == NULL ? "" :
    classNameList[j].instances[k].name,
    *classNameList[j].instances[k].signature,
    ...
    i++) { printf("\tPrimitive Fields%d Name :  , Signature :  , Modifier :  ,
...
static void getLoadedClasses() {
    ...
    for (jclass klass : classes) {
        ...
        /* Creating an object classNameList of all objects in JVM heap is
         * expensive So we limit it to objects belonging to either
        ...
        if (STRING::isStringPresentInList(className, blackPackageName) &&
            !STRING::isStringPresentInList(className,
         ...
         * traversal
         * 1. Every instance of a Class holds a reference to its Class's
         * Class.java object
         * 2. We tag the Class.java object here so we can use this
         * reference/relation to identify the instance's class without
         * additional JNI calls (expensive).
         */
      ...
      cout << "Method Name : " << mInfo->name << " , Signature : " <<
      mInfo->signature
...
void JNICALL exception_thrown(jvmtiEnv *jvmti_env, JNIEnv *jni_env,
    ...
    if (agent->getMethodName(method) ==
        ...
        TIMER_END_(setup_)
        if (inMemory) {
            cQuery = agent->evaluateStringObject(
                    (jstring)agent->getInstanceObjectField(
                            exception, OGO::CYPHER_QUERY_STRING_FIELD,
                            "Ljava/lang/String;"));
            // cout << "Cypher Query String " << cQuery << endl;
        }
        /* Force GC to reduce objects in the heap, this is an optimization that
         * improves the performance of heapIteration and FollowReferences
         * callbacks
         */
        ...
        agent->getInstanceStringArrayField(
                exception, OGO::EXCLUDE_FROM_BLACK_LIST_FIELD,
                "[Ljava/lang/Object;", excludeblackPackageName);
        ...
        for (int j = 0; j < classInfoCount; j++) {
            ...
            for (int i = 0; i < classNameList[j]->instances.size(); i++) {
                classNameList[j]->instances[i]->tag =
                        classNameList[j]->instanceTags[i];
            }
        }
        ...
        fprintf(pFile,
                ...
                DURATION_(writeClassNameListToCsv_), classInfoCount, nodeID,
                objectWrittenToClassNameListCount, DURATION_(total_));
        fclose(pFile);
#endif
...
static void setupCallbacks() {
    ...
    traversalCallbacks.array_primitive_value_callback =
            &heap_arrayFieldCallback;
}
