...
InstanceInfo::InstanceInfo() { clear(); }
...
void InstanceInfo::clear() {
    ...
    references.erase(references.begin(),
                     references.begin() + references.size());
...
void InstanceInfo::writeRelationsToCsv() {
    ...
    if (!references.empty()) {
        ...
        /* Iterate through the referrer instance's referee list and add their
         * tags to the
...
void InstanceInfo::writePropertiesToCsv(Agent *agent) {
    ...
    if (!fields.empty()) {
        ...
        for (const FieldInfo *fInfo : this->fields) {
            if (string properties = fInfo->toString(agent);
                !properties.empty()) {
