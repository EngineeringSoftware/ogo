...
Expression *FunctionInvocationExpr::evaluate(
  ...
  MethodInfo *invokedMethod =
      className2Info.at(callerObjectClass)
          ->getMethodMatchingDescriptor(calledFunction, argDescriptors);
...
Expression *PropertyLookupExpr::evaluate(
  ...
  if (fInfo != NULL) {
    ...
    if (fieldType == Agent::MethodReturnType::JSTRING) {
      fieldValue.l = agent->getInstanceObjectField(callerObject, fInfo->name,
                                                   fInfo->signature);
    } else {
      fieldValue = agent->getInstancePrimitiveField(callerObject, fInfo->name,
                                                    fInfo->signature);
    }
