...
jvalue LiteralExpr::getValue() {
    if(isResolved) {
	    ...
	    value.z = (this->value == "TRUE" | this->value == "true") ? true : false;
            return value;
...
Expression* LiteralExpr::evaluate(const map<string, vector<pair<InstanceInfo *, jobject>>> *pattern) {
...
Expression* FunctionInvocationExpr::evaluate(const map<string, vector<pair<InstanceInfo *, jobject>>> *pattern) {
    jobject callerObject  = pattern->at(callerNode)[0].second;
    string callerObjectClass  = classNameList[pattern->at(callerNode)[0].first->classTag - 1]->name;
    ...
    MethodInfo* invokedMethod = className2Info.at(callerObjectClass)->getMethodMatchingDescriptor(calledFunction,
                                argDescriptors);
    jvalue functionReturn;
    if(invokedMethod != NULL) {
        Agent::MethodReturnType returnType;
        agent->callInstanceMethod(callerObject, invokedMethod->name, invokedMethod->signature, argValues, functionReturn, returnType);
...
PropertyLookupExpr::PropertyLookupExpr(string& node,
                                       string property) {
    this->node = node;
    this->property = property;
}
PropertyLookupExpr::~PropertyLookupExpr() {}
Expression* PropertyLookupExpr::evaluate(const map<string, vector<pair<InstanceInfo *, jobject>>> *pattern) {
    jobject callerObject = pattern->at(node)[0].second;
    string callerObjectClass = classNameList[pattern->at(node)[0].first->classTag - 1]->name;
    FieldInfo* fInfo = pattern->at(node)[0].first->getField(property);
    jvalue fieldValue;
    if(fInfo != NULL) {
        Agent::MethodReturnType fieldType = agent->getFieldType(fInfo->signature);
        if(fieldType == Agent::MethodReturnType::JSTRING) {
            fieldValue.l = agent->getInstanceObjectField(callerObject, fInfo->name, fInfo->signature);
        } else {
            fieldValue = agent->getInstancePrimitiveField(callerObject, fInfo->name, fInfo->signature);
        }
...
BinaryExpr::BinaryExpr(Expression* left,
                       Expression* right,
...
Expression* BinaryExpr::evaluate(const map<string, vector<pair<InstanceInfo *, jobject>>> *pattern) {
...
Expression* UnaryExpr::evaluate(const map<string, vector<pair<InstanceInfo *, jobject>>> *pattern) {
...
COUNT::COUNT() : FunctionInvocationExpr("", "COUNT") {
    this->expr = NULL;
}
COUNT::COUNT(Expression* expr) : FunctionInvocationExpr("", "COUNT") {
    this->expr = expr;
}
Expression* COUNT::evaluate(const map<string, vector<pair<InstanceInfo *, jobject>>> *pattern) {
    if(LiteralExpr* litExpr = dynamic_cast<LiteralExpr*>(expr); litExpr != NULL) {
        return evaluate(litExpr, pattern);
    } else if(PropertyLookupExpr* propExpr = dynamic_cast<PropertyLookupExpr*>(expr); propExpr != NULL) {
        return evaluate(propExpr, pattern);
    }
    return NULL;
}
LiteralExpr *COUNT::evaluate(LiteralExpr *litExpr,
...
LiteralExpr* COUNT::evaluate(PropertyLookupExpr* propExpr,
                             const map<string, vector<pair<InstanceInfo *, jobject>>> *pattern) {
    jlong count = 0;
