...
any MatchClauseVisitor::visitOC_NodePattern(
    ...
    if (ctx->oC_Properties()) {
        AST_NODE_RETURN(
                ctx->oC_Properties()->accept(this), properties,
        ...
        if (nodeLabel.size() == 0 || cInfo->name == nodeLabel) {
            ...
            for (int i = 0; i < tagCount; i++) {
                ...
                if (properties.size() == 0) {
                    ...
                    temp.push_back(
                            pair<InstanceInfo *, jobject>{iInfo, objects[i]});
                ...
                for (const auto &property : properties) {
                    if (property.first == "hash" &&
                        ...
                        vector<pair<InstanceInfo *, jobject>> temp;
                        temp.push_back(pair<InstanceInfo *, jobject>{
                                iInfo, objects[i]});
                        nodeObjectMap->insert({nodeVariable, temp});
                        return any();
                    } else if (isMatch) {
            ...
            if (lhsReturn.has_value() && rhsReturn.has_value()) {
                properties.insert(
                        {any_cast<string>(lhsReturn),
                         any_cast<pair<string, Agent::jvmtiExtendedFieldType>>(
                                 rhsReturn)});
            }
        }
        return properties;
