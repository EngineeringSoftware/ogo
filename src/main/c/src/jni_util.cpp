...
bool Agent::isIntegerType(const enum MethodReturnType &type) {
    return type == MethodReturnType::JBYTE ||
           type == MethodReturnType::JSHORT || type == MethodReturnType::JINT ||
...
#define ENTRY(a, b, c, d)                                                      \
    b *Agent::get##a##ArrayElements(b##Array array) {                          \
        return (*jni_env).functions->Get##a##ArrayElements(jni_env, array,     \
                                                           NULL);              \
    }
JAVA_METHOD_OBJECT_CALLS_
#undef ENTRY
...
jfieldID Agent::getFieldID(jclass fieldDeclaringClass, const string &fieldName,
    ...
    return jni_env->functions->GetFieldID(jni_env, fieldDeclaringClass,
                                          fieldName.c_str(),
...
void Agent::getClassDeclaredFields(jclass klass,
    ...
    jint fieldCount = 0;
    jvmti_env->functions->GetClassFields(jvmti_env, klass, &fieldCount,
                                         &fields);
...
#define ENTRY(a, b, c, d)                                                      \
    case c:                                                                    \
        value.d = (*jni_env).functions->Get##a##Field(jni_env, instance,       \
                                                      fieldID);                \
        return value;
        JAVA_METHOD_OBJECT_CALLS_
#undef ENTRY
...
#define ENTRY(a, b, c)                                                         \
    b Agent::call##a##MethodV(jobject instance, jmethodID methodID, ...) {     \
        va_list args;                                                          \
        return (*jni_env).functions->Call##a##MethodV(jni_env, instance,       \
                                                      methodID, args);         \
    }
JAVA_METHOD_CALLS_
#undef ENTRY
...
#define ENTRY(a, b, c)                                                         \
    b Agent::call##a##MethodA(jobject instance, jmethodID methodID,            \
                              vector<jvalue> &args) {                          \
        return (*jni_env).functions->Call##a##MethodA(jni_env, instance,       \
                                                      methodID, args.data());  \
    }
JAVA_METHOD_CALLS_
#undef ENTRY
...
#define ENTRY(a, b, c, d)                                                      \
    b##Array Agent::create##a##Array(b *array, jint length) {                  \
        printf("Length %d\n", length);                                         \
        b##Array obj =                                                         \
                (*jni_env).functions->New##a##Array(jni_env, length + 1);      \
        return obj;                                                            \
    }
JAVA_METHOD_OBJECT_CALLS_
#undef ENTRY
...
string Agent::evaluateStringObject(jstring stringObject) const {
    string value;
    const char *cString = jni_env->functions->GetStringUTFChars(
            jni_env, stringObject, nullptr);
    if (cString != nullptr) {
...
void Agent::getObjectsWithTags(const long *givenTags, int givenTagsCount,
    ...
    jvmti_env->functions->GetObjectsWithTags(jvmti_env, givenTagsCount,
                                             givenTags, returnedTagsCount,
                                             objects, returnedTags);
}
...
void Agent::iterateThroughHeap(const jvmtiHeapCallbacks *heapOperationCallback,
    ...
    jvmti_env->functions->IterateThroughHeap(jvmti_env, jvmtiHeapFilter,
                                             nullptr, heapOperationCallback,
                                             nullptr);
}
...
void Agent::setObjectPrimitiveField(const string &className, jobject object,
    ...
    case JVMTI_EXTENDED_FIELD_TYPE_BOOLEAN:
        jni_env->functions->SetBooleanField(
                jni_env, object, fieldID, fieldValue == "True" ? true : false);
        break;
    ...
    case JVMTI_EXTENDED_FIELD_TYPE_CHAR:
        jni_env->functions->SetCharField(
                jni_env, object, fieldID,
                static_cast<jchar>(fieldValue.c_str()[0]));
        break;
    case JVMTI_EXTENDED_FIELD_TYPE_SHORT:
        jni_env->functions->SetShortField(
                jni_env, object, fieldID,
    ...
    case JVMTI_EXTENDED_FIELD_TYPE_FLOAT:
        jni_env->functions->SetFloatField(
                jni_env, object, fieldID,
...
void Agent::setObjectReferenceField(jobject referrer, jobject referee,
    ...
    jclass refereeClass =
            (*jni_env).functions->GetObjectClass(jni_env, referee);
    jfieldID fieldID = jni_env->functions->GetFieldID(
...
void Agent::getFieldInfo(const jfieldID field, const jclass klass, string &name,
    ...
    /* Get field modifiers */
    jvmti_env->functions->GetFieldModifiers(jvmti_env, klass, field,
                                            &fModifiers);
