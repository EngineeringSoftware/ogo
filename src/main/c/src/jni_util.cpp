...
Agent::Agent(jvmtiEnv* jvmti_env) {
    this->jvmti_env = jvmti_env;
}

void Agent::setJNIEnv(JNIEnv* jni_env) {
    this->jni_env = jni_env;
}

void Agent::setCapabilities(const jvmtiCapabilities &capabilities) {
    (*jvmti_env).functions->AddCapabilities(jvmti_env,
                                            &capabilities);
}

void Agent::setEventCallbacks(const jvmtiEventCallbacks &callbacks) {
    (*jvmti_env).functions->SetEventCallbacks(jvmti_env,
            &callbacks,
            sizeof(jvmtiEventCallbacks));
    (*jvmti_env).functions->SetEventNotificationMode(jvmti_env,
            JVMTI_ENABLE,
...
jlong Agent::getIntegerTypesAsLong(const jvalue &value,
    ...
    default:
        throw std::runtime_error("Error : type must be JBYTE, JSHORT, JINT or JLONG");
    }
}

bool Agent::isIntegerType(const enum MethodReturnType &type) {
    return type == MethodReturnType::JBYTE ||
           type == MethodReturnType::JSHORT ||
           type == MethodReturnType::JINT ||
           type == MethodReturnType::JLONG;
}

jdouble Agent::getFloatTypesAsDouble(const jvalue &value, const enum util::Agent::MethodReturnType &type) {
    switch(type) {
...
jdouble Agent::getNumberTypeAsDouble(const jvalue &value, const enum MethodReturnType &type) {
...
jlong Agent::getNumberTypeAsLong(const jvalue &value, const enum MethodReturnType &type) {
    if(!isNumberType(type)) {
        throw std::runtime_error("Error : given type must be a number type");
    }
    if(isIntegerType(type)) {
...
bool Agent::isFloatType(const enum MethodReturnType &type) {
    return type == MethodReturnType::JFLOAT ||
           type == MethodReturnType::JDOUBLE;
}
...
Agent::MethodReturnType Agent::getPriorityType(const enum util::Agent::MethodReturnType &type1, const enum util::Agent::MethodReturnType &type2) {
    if(type1 < type2) {
        return type2;
    }
    return type1;
...
void Agent::createRawMonitor() {
    ...
    (*jvmti_env).functions->CreateRawMonitor(jvmti_env,
            "agent data",
            &lock);
}

void Agent::setTag(jobject object,
                   jlong tag) {
    /* Create raw monitor to enter critical sections */
    (*jvmti_env).functions->SetTag(jvmti_env,
                                   object,
                                   tag);
}
...
string Agent::getMethodName(jmethodID methodID) {
    ...
    string methodName = "";
    (*jvmti_env).functions->GetMethodName(jvmti_env,
                                          methodID,
                                          &methodNameC,
                                          NULL,
                                          NULL);
...
jclass Agent::getObjectClass(jobject object) {
    return (*jni_env).functions->GetObjectClass(jni_env,
            object);
}
...
#define ENTRY(a, b, c, d, e) jobject Agent::create##a##Object( b value){ string descriptor = "("STRINGIFY( d )")V";  return createObjectV(#c , descriptor, value );}
JAVA_OBJECT_CREATION_TYPE_
#undef ENTRY

jobject Agent::createObject(const jvalue& value,
                            const MethodReturnType& type) {
    switch(type) {
#define ENTRY(a, b, c, d) case c : return create##a##Object(value. d);
...
jstring Agent::createNewString(string &str) {
    return (*jni_env).functions->NewStringUTF(jni_env,
            str.c_str());
}

jobject Agent::createObject(string className,
                            string constructorDescriptor,
                            vector<jvalue>& args) {
    jclass klass = getClass(className);
    jmethodID methodID = getMethodID(klass,
                                     "<init>",
                                     constructorDescriptor);
    return (*jni_env).functions->NewObject(jni_env,
                                           klass,
                                           methodID,
                                           args.data());
}

jobject Agent::createObjectV(string className,
                             string constructorDescriptor,
                             ...) {
    jclass klass = getClass(className);
    jmethodID methodID = getMethodID(klass,
    ...
    createdObject = (*jni_env).functions->NewObjectV(jni_env,
						     klass,
						     methodID,
						     args);
    va_end(args);
...
jobject Agent::createObjectV(jclass klass,
                             string constructorDescriptor,
                             ...) {
    jmethodID methodID = getMethodID(klass,
                                     "<init>",
...
jobjectArray Agent::createObjectArray(vector<jobject> &elements) {
    ...
    jobjectArray array = (*jni_env).functions->NewObjectArray(jni_env,
                         elements.size(),
                         klass,
                         NULL);
...
void Agent::setObjectArrayElement(jobjectArray objectArray,
                                  jobject element,
                                  jsize index) {
    (*jni_env).functions->SetObjectArrayElement(jni_env,
            objectArray,
            index,
            element);
}

#define ENTRY(a, b, c, d) b* Agent::get##a##ArrayElements(b##Array array){ return (*jni_env).functions->Get##a##ArrayElements(jni_env, array, NULL);}
JAVA_METHOD_OBJECT_CALLS_
#undef ENTRY
...
string Agent::getClassName(jclass klass) {
    ...
    string className = "";
    (*jvmti_env).functions->GetClassSignature(jvmti_env,
            klass,
            &classNameC,
            NULL);
    if(classNameC != NULL) {
...
void Agent::getClassName(jclass klass,
                         string& className,
...
void Agent::getLoadedClasses(vector<jclass> &classes) {
    ...
    jint classCount = 0;
    (*jvmti_env).functions->GetLoadedClasses(jvmti_env,
            &classCount,
            &classesC);
...
jfieldID Agent::getFieldID(jclass fieldDeclaringClass,
                           string fieldName,
                           string fieldDescriptor) {
    return (*jni_env).functions->GetFieldID(jni_env,
                                            fieldDeclaringClass,
                                            fieldName.c_str(),
...
void Agent::getClassDeclaredFields(jclass klass,
    ...
    jint fieldCount = 0;
    (*jvmti_env).functions->GetClassFields(jvmti_env,
                                           klass,
                                           &fieldCount,
                                           &fields);
...
void Agent::getClassDeclaredMethods(jclass klass,
    ...
    jint methodCount = 0;
    (*jvmti_env).functions->GetClassMethods(jvmti_env,
                                            klass,
                                            &methodCount,
                                            &methods);
...
jmethodID Agent::getMethodID(jclass methodDeclaringClass,
                             string methodName,
                             string methodDescriptor) {
    return (*jni_env).functions->GetMethodID(jni_env,
            methodDeclaringClass,
            methodName.c_str(),
            methodDescriptor.c_str());
}

jmethodID Agent::getMethodID(string methodDeclaringClass,
                             string methodName,
                             string methodDescriptor) {
    return getMethodID(getClass(methodDeclaringClass),
                       methodName,
                       methodDescriptor);
}
...
jmethodID Agent::getStaticMethodID(jclass methodDeclaringClass,
				   string methodName,
				   string methodDescriptor) {
    return (*jni_env).functions->GetStaticMethodID(jni_env,
            methodDeclaringClass,
            methodName.c_str(),
            methodDescriptor.c_str());
}
...
jmethodID Agent::getStaticMethodID(string methodDeclaringClass,
				   string methodName,
				   string methodDescriptor) {
    return getStaticMethodID(getClass(methodDeclaringClass),
			     methodName,
			     methodDescriptor);
}
...
Agent::MethodReturnType Agent::getMethodReturnType(string methodDescriptor) {
    string returnType = methodDescriptor.substr(methodDescriptor.find(")") + 1, 1);
    if(returnType == "Ljava/lang/String;") {
        return JSTRING;
    }
    switch (returnType[0]) {
#define ENTRY(a, b) case SINGLE_QUOTES(a): return b;
...
jobject Agent::getInstanceObjectField(jobject instance,
                                      string fieldName,
                                      string fieldDescriptor) {
    jclass instanceClass = getObjectClass(instance);
    jfieldID fieldID = getFieldID(instanceClass,
                                  fieldName,
                                  fieldDescriptor);
    return (*jni_env).functions->GetObjectField(jni_env,
            instance,
            fieldID);
}
...
#define ENTRY(a, b, c, d) case c: value. d = (*jni_env).functions->Get##a##Field(jni_env, instance, fieldID); return value;
        JAVA_METHOD_OBJECT_CALLS_
#undef ENTRY
    case JOBJECT:
    case JNULL:
...
#define ENTRY(a, b, c, d, e) case SINGLE_QUOTES( d ) : return e;
            JAVA_OBJECT_CREATION_TYPE_
#undef ENTRY
        }
...
void Agent::getInstanceObjectArrayField(jobject instance,
    ...
    jobject objectArrayInstance = getInstanceObjectField(instance,
                                  fieldName,
                                  fieldDescriptor);
    jsize arraySize = getArrayLength((jarray) objectArrayInstance);
    for(jsize i = 0; i < arraySize; i++) {
        objectArray.push_back(getObjectArrayElement((jobjectArray) objectArrayInstance, i));
    }
}
...
void Agent::getInstanceStringArrayField(jobject instance,
    ...
    jobjectArray stringArrayInstance = (jobjectArray) getInstanceObjectField(instance,
                                       fieldName,
                                       fieldDescriptor);
...
void Agent::getStringArrayElements(jobjectArray array,
    ...
    for(jsize i = 0; i < arraySize; i++) {
        stringArray.push_back(evaluateStringObject((jstring) getObjectArrayElement(array, i)));
    }
}

jsize Agent::getArrayLength(jarray array) {
    return (*jni_env).functions->GetArrayLength(jni_env,
            array);
}

jobject Agent::getObjectArrayElement(jobjectArray objectArray,
                                     jsize index) {
    return (*jni_env).functions->GetObjectArrayElement(jni_env,
            objectArray,
            index);
}

#define ENTRY(a, b, c) b Agent::call##a##MethodV(jobject instance, jmethodID methodID, ...){ va_list args; return (*jni_env).functions->Call##a##MethodV(jni_env, instance, methodID, args);}
JAVA_METHOD_CALLS_
#undef ENTRY

#define ENTRY(a, b, c) b Agent::call##a##MethodA(jobject instance, jmethodID methodID, vector<jvalue>& args){ return (*jni_env).functions->Call##a##MethodA(jni_env, instance, methodID, args.data());}
JAVA_METHOD_CALLS_
#undef ENTRY

#define ENTRY(a, b, c) b Agent::callStatic##a##MethodA(jclass klass, jmethodID methodID, vector<jvalue>& args){ return (*jni_env).functions->CallStatic##a##MethodA(jni_env, klass, methodID, args.data());}
JAVA_METHOD_CALLS_
#undef ENTRY

void Agent::callInstanceMethod(jobject object,
                               string methodName,
                               string methodDescriptor,
                               vector<jvalue>& args,
                               jvalue& returnValue,
    ...
    switch(returnType) {
#define ENTRY(a, b, c, d) case c : returnValue. d = call##a##MethodA(object, methodID, args); break;
        JAVA_METHOD_OBJECT_CALLS_
#undef ENTRY
    case JVOID:
...
void Agent::callStaticMethod(string className,
                             string methodName,
                             string methodDescriptor,
                             vector<jvalue>& args,
                             jvalue& returnValue,
    ...
    returnType = getMethodReturnType(methodDescriptor);
    switch(returnType) {
#define ENTRY(a, b, c, d) case c : returnValue. d = callStatic##a##MethodA(klass, methodID, args); break;
        JAVA_METHOD_OBJECT_CALLS_
#undef ENTRY
    case JVOID:
...
//printf("Array %p\n", obj);(*jni_env).functions->Set##a##ArrayRegion(jni_env, obj, 0, length, array);
#define ENTRY(a, b, c, d) b##Array Agent::create##a##Array(b *array, jint length) { printf("Length %d\n", length); b##Array obj = (*jni_env).functions->New##a##Array(jni_env, length + 1);return obj;}
JAVA_METHOD_OBJECT_CALLS_
#undef ENTRY
...
string Agent::evaluateStringObject(jstring stringObject) {
    string value = "";
    const char* cString = (*jni_env).functions->GetStringUTFChars(jni_env,
                          stringObject,
                          NULL);
    if(cString != NULL) {
        value.assign(cString);
    }
    (*jni_env).functions->ReleaseStringUTFChars(jni_env,
            stringObject,
            cString);
    return value;
}
...
int Agent::getObjectHashCode(jobject object) {
    jint hashCode = 0;
    (*jvmti_env).functions->GetObjectHashCode(jvmti_env,
            object,
            &hashCode);
    return (int) hashCode;
}

void Agent::getObjectsWithTags(long *givenTags,
                               int givenTagsCount,
                               jobject **objects,
                               long **returnedTags,
                               int *returnedTagsCount) {
    (*jvmti_env).functions->GetObjectsWithTags(jvmti_env,
            givenTagsCount,
            givenTags,
            returnedTagsCount,
            objects,
...
void Agent::rawMonitorEnter() {
    (*jvmti_env).functions->RawMonitorEnter(jvmti_env,
                                            lock);
}

void Agent::rawMonitorExit() {
    (*jvmti_env).functions->RawMonitorExit(jvmti_env,
                                           lock);
}

void Agent::iterateThroughHeap(jvmtiHeapCallbacks *heapOperationCallback,
                               int jvmtiHeapFilter) {
    (*jvmti_env).functions->IterateThroughHeap(jvmti_env,
            jvmtiHeapFilter,
            NULL,
            heapOperationCallback,
            NULL);
}
...
void Agent::followReferences(jvmtiHeapCallbacks *heapOperationCallback,
                             int jvmtiHeapFilter,
                             jobject root) {
    (*jvmti_env).functions->FollowReferences(jvmti_env,
            jvmtiHeapFilter,
            NULL,
            root,
            heapOperationCallback,
...
void Agent::Deallocate(unsigned char** mem) {
    ...
    (*jvmti_env).functions->Deallocate(jvmti_env,
                                       *mem);
    *mem = NULL;
}
 ...
 */
void Agent::setObjectPrimitiveField(string className,
                                    jobject object,
                                    string fieldName,
                                    jvmtiExtendedFieldType type,
                                    string fieldValue) {
    jclass cls = (*jni_env).functions->FindClass(jni_env, className.c_str());
    jfieldID fieldID = (*jni_env).functions->GetFieldID(jni_env, cls, fieldName.c_str(), getUnboxedDescriptorForType(type).c_str());
    switch (type) {
    case JVMTI_EXTENDED_FIELD_TYPE_BOOLEAN :
        (*jni_env).functions->SetBooleanField(jni_env, object, fieldID, fieldValue == "True" ? true : false);
        break;
    case JVMTI_EXTENDED_FIELD_TYPE_BYTE :
        (*jni_env).functions->SetByteField(jni_env, object, fieldID, (jbyte) stoi(fieldValue));
        break;
    case JVMTI_EXTENDED_FIELD_TYPE_CHAR :
        (*jni_env).functions->SetCharField(jni_env, object, fieldID, (jchar) (fieldValue.c_str()[0]));
        break;
    case JVMTI_EXTENDED_FIELD_TYPE_SHORT :
        (*jni_env).functions->SetShortField(jni_env, object, fieldID, (jshort) stoi(fieldValue));
        break;
    case JVMTI_EXTENDED_FIELD_TYPE_INT :
        (*jni_env).functions->SetIntField(jni_env, object, fieldID, (jint) stoi(fieldValue));
        break;
    case JVMTI_EXTENDED_FIELD_TYPE_LONG :
        (*jni_env).functions->SetLongField(jni_env, object, fieldID, (jlong) stol(fieldValue));
        break;
    case JVMTI_EXTENDED_FIELD_TYPE_FLOAT :
        (*jni_env).functions->SetFloatField(jni_env, object, fieldID, (jfloat) stod(fieldValue));
        break;
    case JVMTI_EXTENDED_FIELD_TYPE_DOUBLE :
        (*jni_env).functions->SetDoubleField(jni_env, object, fieldID, (jdouble) stod(fieldValue));
        break;
 ...
 */
void Agent::setObjectReferenceField(jobject referrer,
                                    jobject referee,
                                    string fieldName,
                                    string fieldDescriptor) {
    ...
    jfieldID fieldID = (*jni_env).functions->GetFieldID(jni_env, referrerClass, fieldName.c_str(), fieldDescriptor.c_str());
...
/**
 * @fn          void getFieldInfo(const jfieldID, const jclass, char**, char**, int*)
 * @brief       Get class fields, the jfieldID can be used to get field names later on
 *              (see method assignFieldNames()). This is used to bypass alternative
 *              implementation using jvmti function GetObjectsWithTag which is extremely
 *              inefficient
 *
 * @author      1sand0s
 * @param       field       field ID of field whose information is to be retrieved
 ...
 */
void Agent::getFieldInfo(const jfieldID field,
                         const jclass klass,
                         string& name,
                         string& signature,
                         int& modifiers) {
    ...
    (*jvmti_env).functions->GetFieldName(jvmti_env,
                                         klass,
                                         field,
                                         &cName,
                                         &cSignature,
    ...
    /* Get field modifiers */
    (*jvmti_env).functions->GetFieldModifiers(jvmti_env,
            klass,
            field,
            &fModifiers);
 ...
 * @param       modifiers   modifiers of the field (allocated within function)
 */
void Agent::getMethodInfo(const jmethodID method,
                          string& name,
                          string& signature,
                          int& modifiers) {
    char *cName = NULL;
    ...
    (*jvmti_env).functions->GetMethodModifiers(jvmti_env,
            method,
            &fModifiers);
