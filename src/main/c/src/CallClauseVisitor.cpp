...
CallClauseVisitor::CallClauseVisitor(Agent* agent,
...
any CallClauseVisitor::visitOC_SinglePartQuery(CypherParser::OC_SinglePartQueryContext *ctx) {
...
any CallClauseVisitor::visitOC_MultiPartQuery(CypherParser::OC_MultiPartQueryContext *ctx) {
...
any CallClauseVisitor::visitOC_ReadingClause(CypherParser::OC_ReadingClauseContext *ctx) {
...
any CallClauseVisitor::visitOC_InQueryCall(CypherParser::OC_InQueryCallContext *ctx) {
    if(ctx->CALL()) {
...
any CallClauseVisitor::visitOC_ExplicitProcedureInvocation(CypherParser::OC_ExplicitProcedureInvocationContext *ctx) {
    pair<string, string> caller;
    string methodDescriptor;
    if(ctx->oC_ProcedureName()) {
        AST_NODE_RETURN(ctx->oC_ProcedureName()->accept(this), caller, pair<string,string>)
        caller.second = STRING::cypherLabelToClass(caller.second);
        printf("Caller and Callee %s %s\n", caller.first.c_str(), caller.second.c_str());
        ...
        string className = classNameList[nodeObjectMap->at(caller.first)[0].first->classTag - 1]->name;
        ...
        for(int j = 0; j < ctx->oC_Expression().size(); j++) {
            ...
            argClass.push_back(classNameList[nodeObjectMap->at(arg)[0].first->classTag - 1]->name);
        }
            ...
            if(cInfo->name == className) {
                /*for(MethodInfo* mInfo : cInfo->methods) {
                      if(mInfo->name == caller.second) {
                          int argCount = 0;
                          string str = mInfo->signature.substr(1, mInfo->signature.find(")") - 1);
                          while(str.length() > 0) {
                              int ind = str.find("L");
                              if(ind == 0) {
...
any CallClauseVisitor::visitOC_Expression(CypherParser::OC_ExpressionContext *ctx) {
...
any CallClauseVisitor::visitOC_OrExpression(CypherParser::OC_OrExpressionContext *ctx) {
...
any CallClauseVisitor::visitOC_XorExpression(CypherParser::OC_XorExpressionContext *ctx) {
...
any CallClauseVisitor::visitOC_AndExpression(CypherParser::OC_AndExpressionContext *ctx) {
...
any CallClauseVisitor::visitOC_NotExpression(CypherParser::OC_NotExpressionContext *ctx) {
...
any CallClauseVisitor::visitOC_ComparisonExpression(CypherParser::OC_ComparisonExpressionContext *ctx) {
...
any CallClauseVisitor::visitOC_AddOrSubtractExpression(CypherParser::OC_AddOrSubtractExpressionContext *ctx) {
...
any CallClauseVisitor::visitOC_MultiplyDivideModuloExpression(CypherParser::OC_MultiplyDivideModuloExpressionContext *ctx) {
...
any CallClauseVisitor::visitOC_PowerOfExpression(CypherParser::OC_PowerOfExpressionContext *ctx) {
...
any CallClauseVisitor::visitOC_UnaryAddOrSubtractExpression(CypherParser::OC_UnaryAddOrSubtractExpressionContext *ctx) {
...
any CallClauseVisitor::visitOC_StringListNullOperatorExpression(CypherParser::OC_StringListNullOperatorExpressionContext *ctx) {
...
any CallClauseVisitor::visitOC_PropertyOrLabelsExpression(CypherParser::OC_PropertyOrLabelsExpressionContext *ctx) {
    if(ctx->oC_Atom()) {
...
any CallClauseVisitor::visitOC_ProcedureName(CypherParser::OC_ProcedureNameContext *ctx) {
    string node;
    string method;
...
any CallClauseVisitor::visitOC_Namespace(CypherParser::OC_NamespaceContext *ctx) {
...
any CallClauseVisitor::visitOC_SymbolicName(CypherParser::OC_SymbolicNameContext *ctx) {
