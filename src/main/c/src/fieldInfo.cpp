...
string FieldInfo::toString(Agent* agent) {
	...
	string valueString="";
    switch (type) {
#define ENTRY(a, b, c, d) case Agent::MethodReturnType::c: valueString = std::to_string(value[0]. d); break;
...
#undef ENTRY
    ...
    break;
#define ENTRY(a, b, c, d) case Agent::MethodReturnType::c##_ARRAY: {for(int j = 0; j < this->value.size(); j++){valueString+= std::to_string(this->value[j]. d); if(j < this->value.size() - 1){valueString+=",";}}}break;
JAVA_METHOD_OBJECT_CALLS_
#undef ENTRY
	/*
	...
	if(name.size() > 0 && valueString.size() > 0){
	  sstream << name << "," << Agent::fieldTypeToString(type) << "," << valueString;
	} else{
	  sstream << "";
	}
    return sstream.str();
